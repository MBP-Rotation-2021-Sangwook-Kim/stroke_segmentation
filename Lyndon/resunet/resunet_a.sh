...
#SBATCH --mem=64000M
#SBATCH --time=30:00:0
#SBATCH --out=/scratch/lboone/deepbench/experiments/networks/2/wmh_hab/models/resunet_a/%j.out

# when tested on CC, avg step time for BS=2 (i.e. 2 samples/step) is ~0.78 s
# CC -> Compute Canada, BS -> Batch Size
...
# start training
python /scratch/lboone/deepbench/experiments/networks/2/wmh_hab/models/resunet_a/train.py

# copy output files
cp -r $SLURM_TMPDIR/out /scratch/lboone/deepbench/experiments/networks/2/wmh_hab/models/resunet_a
